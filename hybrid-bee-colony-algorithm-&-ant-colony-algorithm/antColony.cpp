...
int Dist(std::vector <int> path, std::vector <std::vector <edge>> M) // фитнес функция
{
	int res = 0;
...
std::pair <std::vector<int>, int> ant_colony(std::vector <std::vector<int>> Mprev)
{
	...
	int n = Mprev.size();
	int k = 5; // количество муравьев на каждой вершине
	...
	while (Iterations--)
	{
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				M[i][j].UpdatePheromones(); // добавление феромонов и затем испарение
	}
